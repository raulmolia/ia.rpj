 ...
 *  Los spans de color HTML (<span style="color:...">) ya vienen intactos desde smartEscapeHtml
...
function inlineMarkdownToHtml(text: string): string {
    return text
        ...
        .replace(/\*(.+?)\*/g, "<em>$1</em>")
        .replace(/~~(.+?)~~/g, "<del>$1</del>")
    // Note: <span style="color:..."> tags are already valid HTML here (preserved by smartEscapeHtml)
}
...
/** Parsea markdown inline a segmentos con estilo para PDF (incluye spans de color HTML) */
function parseInlineMarkdown(text: string, color?: string): InlineSegment[] {
    ...
    while ((match = regex.exec(text)) !== null) {
        if (match[1] !== undefined) {
            segments.push({ text: match[1], bold: false, italic: false, code: true, color })
        } else if (match[2] !== undefined) {
            segments.push({ text: match[2], bold: true, italic: false, code: false, color })
        } else if (match[3] !== undefined) {
            segments.push({ text: match[3], bold: false, italic: true, code: false, color })
        } else if (match[4] !== undefined) {
            segments.push({ text: match[4], bold: false, italic: false, code: false, color })
        }
    }

    return segments.length > 0 ? segments : [{ text, bold: false, italic: false, code: false, color }]
}

function parseInlineSegments(text: string): InlineSegment[] {
    const segments: InlineSegment[] = []
    // Pre-process: split by <span style="color:..."> HTML color tags first
    const colorSpanRegex = /<span style="color:\s*([^"]+)">(.*?)<\/span>/g
    let lastIndex = 0
    let match: RegExpExecArray | null

    while ((match = colorSpanRegex.exec(text)) !== null) {
        // Process text before this color span
        if (match.index > lastIndex) {
            parseInlineMarkdown(text.slice(lastIndex, match.index)).forEach((s) => segments.push(s))
        }
        // Process the colored content (may also have bold/italic inside)
        const color = match[1].trim()
        parseInlineMarkdown(match[2], color).forEach((s) => segments.push(s))
        lastIndex = match.index + match[0].length
    }

    // Process any remaining text after the last span
    if (lastIndex < text.length) {
        parseInlineMarkdown(text.slice(lastIndex)).forEach((s) => segments.push(s))
    }

    return segments.length > 0 ? segments : [{ text, bold: false, italic: false, code: false }]
}

/** Limpia formato markdown inline y spans HTML de color (para calcular anchos en PDF) */
...
function renderInlineText(
    ...
    for (const seg of segments) {
        ...
        if (seg.color) {
            const hex = seg.color.startsWith("#") ? seg.color : null
            if (hex && hex.length >= 7) {
...
`
    ...
    for (const section of sections) {
        if (section.type === "heading") {
            ...
            htmlContent += `    <h${level}>${inlineMarkdownToHtml(smartEscapeHtml(section.content))}</h${level}>\n`
            ...
            if (section.items) {
                for (const item of section.items) {
                    htmlContent += `        <li>${inlineMarkdownToHtml(smartEscapeHtml(item))}</li>\n`
                }
            }
            ...
            if (inList) { htmlContent += `    </${listType}>\n`; inList = false }
            htmlContent += `    <blockquote><p>${inlineMarkdownToHtml(smartEscapeHtml(section.content))}</p></blockquote>\n`
            ...
            if (inList) { htmlContent += `    </${listType}>\n`; inList = false }
            if (section.content) {
                htmlContent += `    <p>${inlineMarkdownToHtml(smartEscapeHtml(section.content))}</p>\n`
            }
        }
    }
...
/**
 * Like escapeHtml but preserves <span style="color:..."> tags intact.
 * Used for Word export where color spans should remain as valid HTML.
 */
function smartEscapeHtml(text: string): string {
    // Extract color spans, escape the rest, then restore
    const spans: string[] = []
    const withPlaceholders = text.replace(/<span style="color:\s*[^"]*">[^<]*<\/span>/g, (match) => {
        spans.push(match)
        return `\x00SPAN${spans.length - 1}\x00`
    })
    const escaped = escapeHtml(withPlaceholders)
    return escaped.replace(/\x00SPAN(\d+)\x00/g, (_, i) => spans[parseInt(i)])
}
