...
import CanvasEditor, { type CanvasVersion } from "./CanvasEditor"
...
// --- LocalStorage helpers for canvas version persistence ---
const CANVAS_LS_PREFIX = "canvas-history-"
const CANVAS_LS_MAX_AGE_MS = 7 * 24 * 60 * 60 * 1000 // 7 days

function getCanvasStorageKey(conversationId: string | null, messageId: string | null): string {
    const convPart = conversationId || "default"
    const msgPart = messageId || "standalone"
    return `${CANVAS_LS_PREFIX}${convPart}-${msgPart}`
}

interface CanvasSavedHistory {
...
function loadCanvasHistory(key: string, initialContent: string): CanvasSavedHistory | null {
    try {
        const raw = localStorage.getItem(key)
        if (!raw) return null
        const parsed: CanvasSavedHistory = JSON.parse(raw)
        // Don't restore if stale
        if (Date.now() - parsed.savedAt > CANVAS_LS_MAX_AGE_MS) {
            localStorage.removeItem(key)
            return null
        }
        // Restore if initialContent matches ANY saved version (not just the latest).
        // This handles the case where the user navigated to an older version before closing —
        // in that case currentContent (and thus the next initialContent) = older version,
        // while latestContent in storage = newest version, so a strict match would fail.
        const contentMatchesAnyVersion = parsed.versions.some(
            (v: { content: string }) => v.content === initialContent
        )
        if (!contentMatchesAnyVersion) return null
        return parsed
    } catch {
        return null
    }
...
function purgeStaleCanvasHistory() {
    try {
        const keysToRemove: string[] = []
        for (let i = 0; i < localStorage.length; i++) {
            const key = localStorage.key(i)
            if (!key?.startsWith(CANVAS_LS_PREFIX)) continue
            const raw = localStorage.getItem(key)
            if (!raw) continue
            const parsed = JSON.parse(raw) as { savedAt?: number }
            if (!parsed.savedAt || Date.now() - parsed.savedAt > CANVAS_LS_MAX_AGE_MS) {
...
export default function CanvasDialog({
    ...
    // Reset when dialog opens with new content; try to restore saved version history
    useEffect(() => {
        if (open) {
            purgeStaleCanvasHistory()
            setMiniMessages([])
            setMiniInput("")
            ...
            const storageKey = getCanvasStorageKey(conversationId, messageId)
            const saved = loadCanvasHistory(storageKey, initialContent)
            if (saved && saved.versions.length > 1) {
                setRestoredHistory({ versions: saved.versions, currentVersionIndex: saved.currentVersionIndex })
            } else {
    ...
    }, [open, initialContent, conversationId, messageId])
        ...
        <div className="flex h-full flex-col">
            ...
            <ScrollArea className="flex-1 px-4 py-3">
                <div className="space-y-3">
                    {/* Empty state */}
                    {miniMessages.length === 0 && !isTransforming && (
                        <div className="flex flex-col items-center justify-center py-8 text-center">
                            <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-muted">
                                <PenLine className="h-5 w-5 text-muted-foreground" />
                            </div>
                            <p className="text-xs font-medium text-muted-foreground">
                                {t("emptyChatPlaceholder")}
                            </p>
    ...
    const editorPanel = (
        <CanvasEditor
            ...
            initialVersions={restoredHistory?.versions}
            initialVersionIndex={restoredHistory?.currentVersionIndex}
            storageKey={getCanvasStorageKey(conversationId, messageId)}
        />
    )
