...
export default function LoginPage() {
    ...
    const handleRegister = async (e: FormEvent) => {
        ...
        try {
            ...
            if (!res.ok) {
                setRegError(data.message || t("loginPage.register.errorDefault"))
            } else {
                setRegSuccess(true)
            }
        } catch {
            setRegError(t("loginPage.register.connectionError"))
        } finally {
            setRegLoading(false)
        }
    }
    ...
    const handleForgotPassword = async (e: FormEvent) => {
        ...
        try {
            ...
            if (!res.ok) {
                setForgotError(data.message || t("loginPage.forgotPassword.errorDefault"))
            } else {
                setForgotSuccess(true)
            }
        } catch {
            setForgotError(t("loginPage.forgotPassword.connectionError"))
        } finally {
                    ...
                    {regSuccess ? (
                        <div className="space-y-4 py-4 text-center">
                            ...
                            <div className="space-y-1">
                                <p className="font-medium">{t("loginPage.register.successTitle")}</p>
                                <p className="text-sm text-muted-foreground">
                                    {t("loginPage.register.successMessage")}
                                </p>
                            </div>
                        ...
                        <form className="space-y-4 pt-2" onSubmit={handleRegister}>
                            <div className="grid gap-4 sm:grid-cols-2">
                                <div className="space-y-2">
                                    <Label htmlFor="reg-nombre">{t("loginPage.register.firstName")} *</Label>
                                    <Input
                                        id="reg-nombre"
                                        value={regNombre}
                                        onChange={(e) => setRegNombre(e.target.value)}
                                        required
                                        placeholder={t("loginPage.register.firstNamePlaceholder")}
                                    />
                                </div>
                                <div className="space-y-2">
                                    <Label htmlFor="reg-apellidos">{t("loginPage.register.lastName")} *</Label>
                                    <Input
                                        id="reg-apellidos"
                                        value={regApellidos}
                                        onChange={(e) => setRegApellidos(e.target.value)}
                                        required
                                        placeholder={t("loginPage.register.lastNamePlaceholder")}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="reg-telefono">{t("loginPage.register.phone")}</Label>
                                <Input
                                    id="reg-telefono"
                                    type="tel"
                                    value={regTelefono}
                                    onChange={(e) => setRegTelefono(e.target.value)}
                                    placeholder={t("loginPage.register.optional")}
                                />
                            </div>

                            <div className="grid gap-4 sm:grid-cols-2">
                                <div className="space-y-2">
                                    <Label htmlFor="reg-organizacion">{t("loginPage.register.organization")}</Label>
                                    <Input
                                        id="reg-organizacion"
                                ...
                                <div className="space-y-2">
                                    <Label htmlFor="reg-cargo">{t("loginPage.register.position")}</Label>
                                    <Input
                                        id="reg-cargo"
                                        value={regCargo}
                            ...
                            <Button type="submit" className="w-full" disabled={regLoading}>
                                {regLoading ? (
                                    <>
                                        ...
                                        {t("loginPage.register.creating")}
                                    </>
                                ) : (
                                    <>
                                        <UserPlus className="mr-2 h-4 w-4" />
                                        {t("loginPage.register.createButton")}
                                    </>
                                )}
                            </Button>
                        </form>
                ...
                <DialogContent className="sm:max-w-md">
                    ...
                    {forgotSuccess ? (
                        <div className="space-y-4 py-4 text-center">
                            ...
                            <div className="space-y-1">
                                <p className="font-medium">{t("loginPage.forgotPassword.successTitle")}</p>
                                <p className="text-sm text-muted-foreground">
                                    {t("loginPage.forgotPassword.successMessage")}
                                </p>
                            </div>
                        ...
                        <form className="space-y-4 pt-2" onSubmit={handleForgotPassword}>
                            ...
                            <Button type="submit" className="w-full" disabled={forgotLoading}>
                                {forgotLoading ? (
                                    <>
                                        <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                                        {t("loginPage.forgotPassword.sending")}
                                    </>
                                ) : (
                                    <>
                                        <KeyRound className="mr-2 h-4 w-4" />
                                        {t("loginPage.forgotPassword.sendButton")}
                                    </>
                                )}
                            </Button>
                        </form>
